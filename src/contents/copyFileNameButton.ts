...
import { waitForElement, observeElementChanges } from "~src/utils/observe"

export const config: PlasmoCSConfig = {
  matches: ["https://stash.sprocket3.systems/projects/*/pull-requests/*/overview"]
}

export const copyFileNameButton = async () => {
  console.log('copyFileNameButton')
  const copyIcon = `
  ...
  for (let i = 0; i < commentedActivityElements.length; i++) {
    const fileHeader = commentedActivityElements[i].querySelector<HTMLElement>(".file-header")
...
waitForElement(".activities", async () => {
  await copyFileNameButton()
})
observeElementChanges(".activities", copyFileNameButton)
